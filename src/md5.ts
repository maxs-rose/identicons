...
export class Md5 {
	...
	public static hashStr(str: string) {
		return this.#onePassHasher.#start().#appendStr(str).#end() ?? 'Failed to hash';
	}
	...
	static #hexOut: string[] = [];

	// Permanent instance is to use for one-call hashing
	static #onePassHasher = new Md5();

	static #hex(x: Int32Array): string {
	...
	static #md5cycle(x: Int32Array | Uint32Array, k: Int32Array | Uint32Array) {
		let a = x[0];
		let b = x[1];
		let c = x[2];
		let d = x[3];
	...
	#state: Int32Array = new Int32Array(4);
	#buffer: ArrayBuffer = new ArrayBuffer(68);
	...
	#start() {
		this.#dataLength = 0;
		this.#bufferLength = 0;
		this.#state.set(Md5.#stateIdentity);
		return this;
	}
